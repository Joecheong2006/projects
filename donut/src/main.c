...
void set_projection(struct matrix* proj, float n, float f, float fov, float aspect) {
    ...
    float t = tan(fov * PI180);

    proj->data[0][0] = 1 / (t * aspect);
    proj->data[0][1] = 0;
    proj->data[0][2] = 0;
    proj->data[0][3] = 0;
...
void normalize_vec3(struct matrix* v) {
    float r = 1.0 / sqrtf(v->data[0][0] * v->data[0][0] + v->data[1][0] * v->data[1][0] + v->data[2][0] * v->data[2][0]);
    v->data[0][0] *= r;
    v->data[1][0] *= r;
    v->data[2][0] *= r;
}
    ...
    while (looping) {
        ...
        for (float iy = 0; iy < 2 * PI; iy += 0.07) {
            ...
            for (float ix = 0; ix < 2 * PI; ix += angle_step) {
                ...
                float iw = 1.0 / new_pos->data[3][0];
                float x = SX * new_pos->data[0][0] * iw;
                float y = SY * new_pos->data[1][0] * iw;
                float z = new_pos->data[2][0] * iw;
