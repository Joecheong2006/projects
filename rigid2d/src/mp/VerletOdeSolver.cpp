...
void VerletOdeSolver::solve(const real& dt, const std::vector<RigidBody2D*>& objects) {
    for (auto& object : objects) {
        if (object->isStatic)
            continue;

        object->m_position += object->m_velocity * dt + object->m_acceleration * dt * dt * 0.5;
        object->m_velocity = (object->m_oacceleration + object->m_acceleration) * dt * 0.5 + glm::pow(1.0 - object->m_damping, dt) * object->m_velocity;
